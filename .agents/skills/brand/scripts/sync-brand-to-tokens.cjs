...
function generateColorScale(baseHex, darkHex, lightHex) {
    ...
    return {
        50: {
            $value: lightHex || adjustBrightness(baseHex, 0.9),
            $type: "color",
        },
        100: {
            $value: lightHex || adjustBrightness(baseHex, 0.8),
            $type: "color",
        },
...
function adjustBrightness(hex, percent) {
    ...
    const r = Math.min(
        255,
        Math.max(0, (num >> 16) + Math.round(255 * percent)),
    );
    const g = Math.min(
        255,
