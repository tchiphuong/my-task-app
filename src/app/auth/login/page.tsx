    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-linear-to-tr from-primary-500/10 via-background to-secondary-500/10 px-4 relative">
      {isZaloOrFb && (
        <div className="absolute top-4 left-4 right-4 z-50 p-4 rounded-2xl border border-warning-200 bg-warning-50/90 backdrop-blur-md shadow-lg flex gap-3 animate-appearance-in">
          <Icon icon="lucide:info" className="text-warning-600 text-xl shrink-0 mt-0.5" />
          <div className="flex flex-col gap-1 text-left">
            <h4 className="font-semibold text-warning-800 text-sm">
              Bạn đang mở trong Zalo hoặc Facebook
            </h4>
            <p className="text-warning-700 text-xs leading-relaxed">
              Trình duyệt này có thể chặn đăng nhập Google. Vui lòng bấm vào biểu tượng <strong>ba chấm (...)</strong> hoặc <strong>la bàn</strong> ở góc màn hình và chọn <strong>&quot;Mở bằng trình duyệt&quot;</strong> để tiếp tục nhé!
            </p>
          </div>
        </div>
      )}
        ...
        <Card.Content className="flex flex-col gap-6 py-6">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-6 gap-2">
              ...
              <span className="text-xs text-default-500 font-medium">{t("auth.preparing")}</span>
            </div>
          ) : (
