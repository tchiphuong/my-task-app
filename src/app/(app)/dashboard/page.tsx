...
export default function DashboardPage() {
    ...
    const overdueCount = tasks.filter(
        (t) =>
            !t.isDaily && t.status !== TASK_STATUS.DONE && t.dueDate < todayStr,
    ).length;
    ...
    const completedRegularToday = regularTasksToday.filter(
    ...
    const chartData = Array.from({ length: 7 }).map((_, index) => {
        ...
        const dayStr = format(day, DATE_FORMATS.DATE);
        const dayName = format(day, DATE_FORMATS.WEEKDAY, { locale: vi }); // T2, T3...
    ...
    const getGreetingIcon = () => {
        const hour = new Date().getHours();
        const className =
            "w-6 h-6 ml-2 text-warning-500 inline-block align-middle";
        if (hour < 12) return <Sun className={className} />;
        if (hour < 18) return <CloudSun className={className} />;
        return <Moon className={className} />;
    };

    // Logic chọn ngẫu nhiên câu thoại của Mascot Bơ Sữa
    const { mascotSpeech } = useMemo(() => {
        let key: string = MASCOT_STATUS.IN_PROGRESS;
        let count = totalTasksToday - completedTodayCount;

        if (overdueCount > 0) {
            key = MASCOT_STATUS.OVERDUE;
            count = overdueCount;
        } else if (totalTasksToday === 0) {
            key = MASCOT_STATUS.NO_TASKS;
        } else if (progressPercent === 100) {
            key = MASCOT_STATUS.ALL_DONE;
        }

        const quotes = t(`dashboard.mascot.${key}`, {
            returnObjects: true,
            count,
    ...
    const getProgressHelperText = () => {
        ...
        return t("dashboard.progressHelperText3", {
            count: totalTasksToday - completedTodayCount,
        });
    };
    ...
    return (
        <div className="flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-default-500 mt-0.5 font-medium">
                            {t("dashboard.readyText", {
                                date: format(new Date(), DATE_FORMATS.DISPLAY, {
                                    locale: vi,
                                }),
                            })}
                        </p>
            ...
            {/* Lời khuyên/Gợi ý cổ vũ (Ẩn Mascot icon theo yêu cầu) */}
            {mascotSpeech && (
                <Card className="border-2 border-b-6 border-default-200 bg-white dark:bg-zinc-900 rounded-2xl shadow-none">
                    <Card.Content className="p-4 flex items-center gap-3">
                        <span className="text-xl">💡</span>
                        <p className="text-xs md:text-sm font-bold text-default-900 leading-relaxed">
                            {mascotSpeech}
                        </p>
                    </Card.Content>
                </Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Card className="border-2 border-b-6 border-warning/20 bg-warning/5 dark:bg-warning/10 rounded-2xl shadow-none overflow-hidden relative">
                    ...
                    <Card.Content className="flex flex-col gap-2 p-5 justify-between relative z-10">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-warning/80 uppercase tracking-wide">
                                {t("dashboard.streakTitle")}
                            </span>
                            <FireIcon
                                className={`w-5 h-5 transition-all duration-300 ${streak.currentStreak > 0 ? "text-warning animate-bounce" : "text-default-400"}`}
                            />
                        </div>
                        <div className="flex items-baseline gap-2 mt-2">
                            <span
                                className={`text-3xl font-black ${streak.currentStreak > 0 ? "text-warning animate-pulse" : "text-default-400"}`}
                            >
                                <AnimatedNumber value={streak.currentStreak} />{" "}
                                {t("dashboard.streakDay", { count: "" }).trim()}
                            </span>
                        </div>
                        <p className="text-xs text-warning/80 leading-normal font-semibold">
                            {t("dashboard.bestStreak", {
                                count: streak.bestStreak,
                            })}
                        </p>
                ...
                <Card className="border-2 border-b-6 border-danger/20 bg-danger/5 dark:bg-danger/10 rounded-2xl shadow-none overflow-hidden relative">
                    ...
                    <Card.Content className="flex flex-col gap-2 p-5 justify-between relative z-10">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-danger/80 uppercase tracking-wide">
                            ...
                            {urgentTasks.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-16 text-center text-default-400">
                                    ...
                                    <p className="text-sm font-semibold text-success-600">
                                        {t("dashboard.urgentNoTasksTitle")}
                                    </p>
                                    <p className="text-xs text-default-400 mt-1">
                                        {t("dashboard.urgentNoTasksDesc")}
                                    </p>
                                </div>
                            ) : (
                                            ...
                                            >
                                                <div className="flex flex-col gap-1 w-3/4">
                                                    ...
                                                    <div className="flex items-center gap-1.5 flex-wrap">
                                                        ...
                                                        {task.dueDate && (
                                                            <span
                                                                className={`text-2xs flex items-center gap-1 font-medium ${isOverdue ? "text-danger-500 font-bold" : "text-default-400"}`}
                                                            >
                        ...
                        <div className="w-full h-70">
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    />
                                    <Bar
                                        dataKey={t("dashboard.chartTaskKey")}
                                        fill="#8b5cf6"
                                        radius={[4, 4, 0, 0]}
                                    />
                                    <Bar
                                        dataKey={t("dashboard.chartHabitKey")}
                                        fill="#14b8a6"
                                        radius={[4, 4, 0, 0]}
                                    />
                                </BarChart>
