...
export default function AppLayout({ children }: Readonly<AppLayoutProps>) {
  ...
  const checkAndUpdateSystemState = useTaskStore((state) => state.checkAndUpdateSystemState);
  ...
  if (!mounted || !user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background gap-3">
        ...
        <span className="text-xs text-default-500 font-medium">Đang tải dữ liệu của bạn...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row overflow-hidden relative">
      ...
      <div
        ...
        style={{ top: "-10%", left: "-10%", width: "50%", height: "50%", filter: "blur(100px)" }}
      />
      <div
        className="absolute rounded-full bg-danger/10 dark:bg-danger/5 pointer-events-none z-0"
        style={{ bottom: "10%", right: "-10%", width: "60%", height: "60%", filter: "blur(120px)" }}
      />
