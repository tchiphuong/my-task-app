...
export default function TasksPage() {
    ...
    const [selectedPriority, setSelectedPriority] = useState(FILTER_ALL);

    // Tab trạng thái trên mobile
    const [activeTab, setActiveTab] = useState<string>(TASK_STATUS.TODO);
    ...
    const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        const taskData = {
            ...
            priority: (data.priority ||
                TASK_PRIORITY.MEDIUM) as Task["priority"],
            category: (data.category ||
                TASK_CATEGORIES[0].name) as Task["category"],
            dueDate:
                (data.dueDate as string) ||
                format(new Date(), DATE_FORMATS.DATE),
            isDaily: data.isDaily === "true",
            status: (editingTask
                ? editingTask.status
                : TASK_STATUS.TODO) as Task["status"],
        };
    ...
    const handleTaskMenuAction = (task: Task, actionKey: string) => {
        if (actionKey === "edit") {
            ...
            updateTask(task.id, { status: TASK_STATUS.IN_PROGRESS });
        } else if (actionKey === "stop") {
            updateTask(task.id, { status: TASK_STATUS.TODO });
    ...
    const renderTaskCard = (task: Task) => {
        const isOverdue =
            task.dueDate &&
            task.dueDate < format(new Date(), DATE_FORMATS.DATE) &&
            task.status !== TASK_STATUS.DONE;
        const formattedDueDate = task.dueDate
            ? format(parseISO(task.dueDate), DATE_FORMATS.SHORT_DISPLAY)
            : "";
        ...
        return (
            <Card
                ...
                className={`hover:-translate-y-0.5 ${
                    isOverdue
                        ? "border-danger-200 dark:border-danger-900/30 bg-danger-50/10"
                        : ""
                }`}
            >
                <Card.Content className="p-4 flex flex-col gap-3">
                    <div className="flex justify-between items-start gap-2">
                        <h3
                            className={`font-bold text-sm text-default-900 line-clamp-2 text-left ${task.status === TASK_STATUS.DONE ? "line-through text-default-400" : ""}`}
                        >
                            {task.title}
                        </h3>
                        <Dropdown>
                    ...
                    <div className="flex justify-between items-center border-t border-default-50 dark:border-default-100/10 pt-2.5 mt-1">
                        <span
                            className={`text-2xs flex items-center gap-1 font-semibold ${isOverdue ? "text-danger-500 font-bold" : "text-default-400"}`}
                        >
                            <CalendarIcon className="w-3.5 h-3.5" />
                            {isOverdue
                                ? t("tasks.overdue", { date: formattedDueDate })
                                : formattedDueDate}
                        </span>
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-xl md:text-2xl font-black text-default-900 tracking-tight">
                        {t("menu.tasks")}
                    </h1>
                    ...
                    <div className="w-32">
                        <Select
                            ...
                            {...{
                                ...
                                onSelectionChange: (key: React.Key | null) =>
                                    setSelectedPriority(key as string),
                            }}
                        >
                            <Select.Trigger>
                                <span className="mr-2 flex items-center justify-center text-default-400">
                ...
                >
                    <Tabs.ListContainer>
                        <Tabs.List aria-label={t("tasks.form.status.title")}>
                            <Tabs.Tab id={TASK_STATUS.TODO}>
                                <span className="flex items-center gap-1 text-default-500">
                                    ...
                                    <span className="text-xs">
                                        (
                                        {
                                            getTasksByStatus(TASK_STATUS.TODO)
                                                .length
                                        }
                                        )
                                    </span>
                            ...
                            <Tabs.Tab id={TASK_STATUS.IN_PROGRESS}>
                            ...
                            <Tabs.Tab id={TASK_STATUS.DONE}>
                                <span className="flex items-center gap-1 text-success-500">
                                    ...
                                    <span className="text-xs">
                                        (
                                        {
                                            getTasksByStatus(TASK_STATUS.DONE)
                                                .length
                                        }
                                        )
                                    </span>
                    ...
                    <Tabs.Panel id={TASK_STATUS.TODO}>
                        <div className="flex flex-col gap-3 mt-4">
                            {getTasksByStatus(TASK_STATUS.TODO).length === 0 ? (
                                <div className="text-center py-12 text-default-400 text-xs">
                                    {t("tasks.noTasksTodo")}
                                </div>
                            ) : (
                                getTasksByStatus(TASK_STATUS.TODO).map(
                                    renderTaskCard,
                                )
                            )}
                        </div>
                    </Tabs.Panel>
                    <Tabs.Panel id={TASK_STATUS.IN_PROGRESS}>
                        <div className="flex flex-col gap-3 mt-4">
                            {getTasksByStatus(TASK_STATUS.IN_PROGRESS)
                                .length === 0 ? (
                                <div className="text-center py-12 text-default-400 text-xs">
                                    {t("tasks.noTasksDoing")}
                                </div>
                            ) : (
                                getTasksByStatus(TASK_STATUS.IN_PROGRESS).map(
                                    renderTaskCard,
                                )
                            )}
                        </div>
                    </Tabs.Panel>
                    <Tabs.Panel id={TASK_STATUS.DONE}>
                        <div className="flex flex-col gap-3 mt-4">
                            {getTasksByStatus(TASK_STATUS.DONE).length === 0 ? (
                                <div className="text-center py-12 text-default-400 text-xs">
                                    {t("tasks.noTasksCompleted")}
                                </div>
                            ) : (
                                getTasksByStatus(TASK_STATUS.DONE).map(
                                    renderTaskCard,
                                )
                            )}
                        </div>
                ...
                <div className="flex flex-col gap-3 bg-default-50/50 p-4 rounded-2xl border border-default-100/50 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center px-1 mb-1 relative z-10">
                        ...
                        <Chip size="sm" variant="soft">
                            {getTasksByStatus(TASK_STATUS.TODO).length}
                        </Chip>
                    </div>
                    <div className="flex flex-col gap-3 max-h-150 overflow-y-auto pr-1 relative z-10">
                        {getTasksByStatus(TASK_STATUS.TODO).length === 0 ? (
                            <div className="text-center py-12 text-default-400 text-xs border-2 border-dashed border-default-100 rounded-xl">
                                {t("tasks.dragTodo")}
                            </div>
                        ) : (
                            getTasksByStatus(TASK_STATUS.TODO).map(
                ...
                <div className="flex flex-col gap-3 bg-default-50/50 p-4 rounded-2xl border border-default-100/50 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center px-1 mb-1 relative z-10">
                        ...
                        <Chip size="sm" variant="soft" color="accent">
                            {getTasksByStatus(TASK_STATUS.IN_PROGRESS).length}
                        </Chip>
                    </div>
                    <div className="flex flex-col gap-3 max-h-150 overflow-y-auto pr-1 relative z-10">
                        {getTasksByStatus(TASK_STATUS.IN_PROGRESS).length ===
                ...
                <div className="flex flex-col gap-3 bg-default-50/50 p-4 rounded-2xl border border-default-100/50 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center px-1 mb-1 relative z-10">
                        ...
                        <Chip size="sm" variant="soft" color="success">
                            {getTasksByStatus(TASK_STATUS.DONE).length}
                        </Chip>
                    </div>
                    <div className="flex flex-col gap-3 max-h-150 overflow-y-auto pr-1 relative z-10">
                        {getTasksByStatus(TASK_STATUS.DONE).length === 0 ? (
                ...
                <Modal.Container>
                    <Modal.Dialog className="sm:max-w-md">
                                ...
                                <div className="w-full">
                                    <TextField
                                        ...
                                        defaultValue={
                                            ...
                                            TASK_CATEGORIES[0].name
                                        }
                                    >
                                        <Label>{t("tasks.category")}</Label>
                                        <Input
                                            placeholder={t(
                                                "tasks.form.categoryPlaceholder",
                                            )}
                                        />
                                        <FieldError />
                                    </TextField>
                        ...
                        <Modal.Footer className="w-full">
                            <Button
                                variant="ghost"
                                className="font-semibold text-xs text-danger"
