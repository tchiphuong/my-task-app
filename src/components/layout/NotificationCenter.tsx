...
export default function NotificationCenter() {
    ...
    const markNotificationAsRead = useTaskStore(
        (state) => state.markNotificationAsRead,
    );
    const markAllNotificationsAsRead = useTaskStore(
    ...
    const getNotifIcon = (type: string) => {
        switch (type) {
            ...
            case "warning":
                return (
                    <ExclamationTriangleIcon className="w-8 h-8 text-warning shrink-0" />
                );
            default:
                return (
    ...
    const getBellIcon = () => {
        ...
        return (
            <BellOutline className="w-6 h-6 text-default-500 hover:text-default-700" />
        );
    };
                            ...
                            <div className="flex flex-col w-full divide-y divide-default-100">
                                {notifications.map((notif) => (
                                    <button
                                        ...
                                        className={`w-full flex gap-3 p-4 cursor-pointer transition-colors duration-150 text-left hover:bg-default-50 dark:hover:bg-default-100/20 ${
                                            !notif.read
                                                ? "bg-primary-50/30 dark:bg-primary-950/10 font-medium"
                                                : ""
                                        }`}
                                    >
                                        ...
                                        <div className="flex flex-col gap-1 w-full overflow-hidden">
                                            <p
                                                className={`text-xs text-default-900 line-clamp-2 ${!notif.read ? "font-semibold" : ""}`}
                                            >
                                                {notif.title}
                                            </p>
