...
interface TaskStore {
  ...
  // Task Actions
  addTask: (taskData: Omit<Task, "id" | "createdAt" | "completedDates">) => Promise<void>;
...
const countConsecutiveDays = (completedGoalDates: string[], startDate: Date): number => {
  let streak = 0;
...
const calculateStreak = (
  ...
  if (dailyTasks.length > 0) {
    ...
    if (completedGoalDates.includes(todayStr) || completedGoalDates.includes(yesterdayStr)) {
      const startDate = completedGoalDates.includes(todayStr) ? new Date() : subDays(new Date(), 1);
    ...
    if (daysDiff > 1) {
      ...
      if (isFirebaseConfigured && db && email !== "guest") {
        try {
          ...
          console.error("Lỗi cập nhật streak đứt trên Firestore:", err);
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        set((state: any) => {
      ...
      return {
        ...
        syncWithFirestore: () => {
          ...
          const unsubscribeTasks = onSnapshot(tasksQuery, (snapshot) => {
            ...
            set((state) => {
              const account = state.accounts[email] || { ...initialAccountData };
              return {
                accounts: {
                  ...state.accounts,
                  [email]: {
          ...
          const unsubscribeUser = onSnapshot(userDocRef, (docSnap) => {
            if (docSnap.exists()) {
              ...
              set((state) => {
                ...
                return {
                  accounts: {
                    ...
                    [email]: {
                      ...account,
                      streak: userData.streak || { ...initialAccountData.streak },
              ...
              }).catch((e) => console.error("Lỗi tạo user doc mặc định:", e));
            }
          }, (err) => {
        ...
        addTask: async (taskData) => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              const tasksCollectionRef = collection(db, "users", email, "tasks");
            ...
            set((state) => {
              ...
              const newTask: Task = {
                ...
                id: generateUniqueId(),
                createdAt: new Date().toISOString(),
                completedDates: taskData.isDaily ? [] : undefined,
              };
        ...
        updateTask: async (taskId, updatedFields) => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              const taskDocRef = doc(db, "users", email, "tasks", taskId);
              
              // Lấy thông tin trạng thái cũ
              const account = get().accounts[email];
              const oldTask = account?.tasks.find(t => t.id === taskId);
              
              let completedAt: string | null = null;
              if (updatedFields.status === "done") {
                completedAt = oldTask?.status !== "done" ? new Date().toISOString() : (oldTask?.completedAt || null);
              }
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              ...
              console.error("Lỗi cập nhật task trên Firestore:", err);
            }
          } else {
            // Local fallback
            set((state) => {
              ...
              const updatedTasks = account.tasks.map((task) => {
                if (task.id === taskId) {
                  ...
                  if (updatedFields.status === "done") {
                    completedAt = task.status !== "done" ? new Date().toISOString() : task.completedAt;
                  }

                  return { ...task, ...updatedFields, completedAt };
                }
                return task;
              });
        ...
        deleteTask: async (taskId) => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              ...
              get().addNotification(
                "Đã xóa công việc",
                "Công việc đã được loại bỏ khỏi danh sách.",
                "info"
              );
            } catch (err) {
        ...
        toggleDailyTaskDate: async (taskId, dateStr) => {
          const email = getActiveEmail();
          const account = get().accounts[email] || { ...initialAccountData };
          const targetTask = account.tasks.find((t) => t.id === taskId);
          ...
          if (dateStr === format(new Date(), "yyyy-MM-dd")) {
            updatedStatus = !isCompletedOnDate ? "done" : "todo";
          }

          // Tính toán lại Streak động dựa trên lịch sử
          const dailyTasks = account.tasks.map((t) => 
            t.id === taskId 
              ? { ...t, completedDates: newDates, status: updatedStatus }
              : t
          ).filter((t) => t.isDaily);
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              ...
              console.error("Lỗi toggle daily task trên Firestore:", err);
            }
          } else {
            // Local fallback
            set((state) => {
              const acc = state.accounts[email] || { ...initialAccountData };
              const updatedTasks = acc.tasks.map((task) => {
                if (task.id === taskId) {
                  return { ...task, completedDates: newDates, status: updatedStatus };
                }
                return task;
              });
          ...
          const activeDailyTasks = updatedAccount.tasks.filter((t) => t.isDaily);
          const completedDailyCount = activeDailyTasks.filter((t) => 
            t.completedDates?.includes(dateStr)
          ).length;

          if (activeDailyTasks.length > 0 && completedDailyCount === activeDailyTasks.length) {
            get().addNotification(
              "Hoàn thành xuất sắc mục tiêu ngày!",
              `Chúc mừng! Bạn đã hoàn thành toàn bộ ${activeDailyTasks.length} thói quen ngày hôm nay. Streak của bạn hiện là ${newStreak.currentStreak} ngày! 🔥`,
              "success"
            );
          }
        },
        ...
        addNotification: async (title, message, type) => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              ...
              const updatedNotifications = [newNotification, ...(account?.notifications || [])].slice(0, 50); // Lưu tối đa 50 thông báo gần nhất
              
              await updateDoc(userDocRef, {
                notifications: updatedNotifications,
              });
            } catch (err) {
              console.error("Lỗi gửi notification lên Firestore:", err);
            }
          } else {
            // Local fallback
            set((state) => {
              ...
              return {
                accounts: {
                  ...
                  [email]: {
                    ...account,
                    notifications: [newNotification, ...account.notifications].slice(0, 50),
                  },
                },
              };
            });
        ...
        markNotificationAsRead: async (notificationId) => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              const userDocRef = doc(db, "users", email);
              const account = get().accounts[email];
              const updatedNotifications = (account?.notifications || []).map((n) => 
              ...
              console.error("Lỗi đọc notification trên Firestore:", err);
            }
          } else {
            // Local fallback
            set((state) => {
        ...
        markAllNotificationsAsRead: async () => {
          ...
          if (isFirebaseConfigured && db && email !== "guest") {
            try {
              ...
              console.error("Lỗi đọc tất cả notifications trên Firestore:", err);
            }
          } else {
            // Local fallback
            set((state) => {
        ...
        checkAndUpdateSystemState: async () => {
          ...
          const todayStr = format(new Date(), "yyyy-MM-dd");
          const overdueTasks = account.tasks.filter((t) => 
            !t.isDaily && t.status !== "done" && t.dueDate < todayStr
          );

          if (overdueTasks.length > 0) {
            const hasOverdueNotifToday = account.notifications.some(
              (n) => n.type === "overdue" && n.createdAt.startsWith(todayStr)
            );
            if (!hasOverdueNotifToday) {
              get().addNotification(
                "Có việc trễ hạn chót!",
                `Bạn có ${overdueTasks.length} công việc chưa hoàn thành đã quá hạn chót. Mau kiểm tra lại và giải quyết gấp nhé.`,
                "overdue"
              );
            }
          }
          ...
          await handleStreakBreak(email, account, set, get().addNotification);
        },
      };
    },
    {
